...
export function CustomerDetailPageContent() {
  ...
  return <main className="min-h-full bg-background text-foreground"><div className="mx-auto w-full max-w-[1500px] space-y-6 p-4 sm:p-6 lg:p-8"><Button type="button" variant="ghost" className="min-h-11 gap-2 px-0" onClick={() => navigate(`/shops/${data.shopSlug}/admin/customers`)}><ArrowLeft aria-hidden="true" />{t("detail.back")}</Button><CustomerDetailHeader customer={customer} locale={locale} /><CustomerValueSummary customer={customer} locale={locale} /><div className="grid gap-5 xl:grid-cols-[minmax(0,1.3fr)_minmax(320px,.7fr)]"><div className="space-y-5"><UpcomingAppointments customer={customer} locale={locale} /><VisitHistory customer={customer} locale={locale} /></div><div className="space-y-5"><RetentionSection customer={customer} locale={locale} /><RelationshipSection customer={customer} /></div></div></div></main>;
}
...
function CustomerValueSummary({ customer, locale }: { customer: CustomerDetail; locale: string }) {
  const { t } = useTranslation("customers");
  const values = [["lifetimeSpend", formatCurrency(customer.totalSpent, locale)], ["visits", customer.totalVisits.toLocaleString()], ["averageSpend", formatCurrency(customer.averageSpend, locale)], ["lastVisit", formatCustomerDate(customer.lastVisitAt, locale)]] as const;
...
function VisitHistory({ customer, locale }: { customer: CustomerDetail; locale: string }) {
  const { t } = useTranslation("customers");
  return <Card className="gap-0 overflow-hidden py-0 shadow-xs"><CardHeader className="p-5 pb-3"><CardTitle className="flex items-center gap-2 text-base"><History className="size-4 text-primary" aria-hidden="true" />{t("detail.historyTitle")}</CardTitle></CardHeader><CardContent className="p-0"><div className="divide-y">{customer.appointments.length ? customer.appointments.map((appointment) => <AppointmentHistoryRow key={appointment.id} appointment={appointment} locale={locale} />) : <p className="p-5 text-sm text-muted-foreground">{t("detail.noHistory")}</p>}</div></CardContent></Card>;
}

function AppointmentHistoryRow({ appointment, locale }: { appointment: CustomerAppointment; locale: string }) {
  const { t } = useTranslation("customers");
  return <div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="text-sm font-semibold">{formatCustomerDateTime(appointment.date, appointment.startTime, locale)}</p><p className="mt-1 truncate text-sm text-muted-foreground">{appointment.services.map((service) => service.serviceName).join(", ") || t("detail.noServices")}</p><p className="mt-1 text-xs text-muted-foreground">{appointment.staff?.name ?? t("detail.unassigned")} · {appointment.status}</p></div><div className="flex items-center gap-3 sm:text-right"><div><p className="text-sm font-semibold tabular-nums">{formatCurrency(appointment.totalAmount, locale)}</p>{appointment.payment && <p className="mt-1 text-xs text-muted-foreground">{t(`paymentStatus.${appointment.payment.status.toLowerCase()}`)} · {appointment.payment.method}</p>}</div><CreditCard className="size-4 text-muted-foreground" aria-hidden="true" /></div></div>;
}
