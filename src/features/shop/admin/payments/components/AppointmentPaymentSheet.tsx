...
interface Props {
  ...
  onSuccess: (payment: AppointmentPayment) => Promise<void>;
...
export function AppointmentPaymentSheet({ shopSlug, appointment, staffName, payment, open, onOpenChange, onSuccess, onRefresh }: Props) {
  ...
  const amountDue = payment?.amount ?? appointment.totalAmount;

  const submit = async () => {
    if (!appointment.id || amountDue <= 0 || !method) {
    ...
    const result = payment
      ? await confirmExisting(appointment.id, payment, trimmedNote)
      : await createAndConfirm(appointment.id, {
          method,
          note: trimmedNote,
        });
        ...
        >
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-6">
            ...
            <Field>
              ...
              <Textarea id="appointment-payment-note" value={note} maxLength={500} disabled={isSubmitting} placeholder={t("payments:appointmentFlow.notePlaceholder")} onChange={(event) => setNote(event.target.value)} />
            </Field>

            {validationError ? <Alert variant="destructive"><AlertCircle aria-hidden="true" /><AlertDescription>{t("payments:appointmentFlow.invalid")}</AlertDescription></Alert> : null}
          ...
          <SheetFooter className="border-t bg-background px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div><p className="text-xs text-muted-foreground">{t("payments:detail.total")}</p><p className="text-xl font-semibold tabular-nums">{formatCurrency(amountDue, i18n.language)}</p></div>
            <div className="grid w-full grid-cols-2 gap-2 sm:w-auto">
              ...
              <Button type="submit" className="h-11 min-w-36" disabled={isSubmitting || amountDue <= 0} aria-busy={isSubmitting}>
